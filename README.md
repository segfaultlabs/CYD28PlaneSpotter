...
- **Animated weather corner** — a nicer corner widget than the static icon: sun with slowly rotating rays, drifting clouds, rain; cross-fades between states as Open-Meteo conditions change. Done as small region updates inside the existing staged redraw, so it costs almost nothing (2Hz furniture, not per-frame work).
- **Phosphor afterglow on the sweep** — the line leaves a short fading green trail behind it, like a real CRT radar. Pure vibe, big payoff. (S)
