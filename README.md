...
- ~~**Much better WiFi management**~~ — **DONE**: up to 4 saved networks (web-editable, NVS-persisted, slot 0 seeded from config.env), non-blocking reconnect state machine (radar keeps running on stale data during outages), fallback "PlaneSpotter-Setup" setup AP with on-screen instructions when nothing is reachable (config page at 192.168.4.1, STA retries continue in background), SSID/IP/AP status on the Weather & System screen.
...
- ~~**Map-based location picker for the setup page**~~ — **DONE** (Leaflet + OSM via CDN, click-to-set, range-circle preview, geolocation button). Original note: — setting home lat/lon by typing numbers is clunky. The config page is served by the device but rendered in the user's browser (which has internet), so we can embed **Leaflet from a CDN + OpenStreetMap tiles** at zero device-side cost beyond a few KB of HTML/JS: click on the map fills the lat/lon fields, a "use my current location" button calls the browser's Geolocation API, and the radar range circle can be previewed on the map at the chosen point. Plain number fields stay as a fallback for offline browsers.
- ~~**Device URL + QR code on the system screen**~~ — **DONE** (vendored Nayuki qrcodegen; URL as text + scannable QR, AP-aware). Original note: — the Weather & System screen should show `http://<device-ip>/` as text and as a QR code (vendor Nayuki's single-file `qrcodegen` C library, MIT, render modules as scaled rects) so a phone can jump to the config page without typing an IP. Same mechanism serves the WiFi-provisioning-QR roadmap item below.
- ~~**Smart traffic filter / watchlist system**~~ — **DONE** (5 rules: callsign/reg/type prefix x highlight/hide/only/alert + color, quiet mode, alert banner). Original note: — a small, logical rule engine instead of one-off toggles. Each rule has a **match** (callsign prefix, registration, type code, airline code, or size class — light/small vs heavy via ADS-B category) and an **action**: `highlight` (custom color for blip + trail), `hide` (don't draw), `only` (draw exclusively), `alert` (change the screen accent/background theme or show a banner while a match is airborne). Precedence: `only` > `hide` > `highlight` > default. Plus a **quiet mode**: show normal traffic only when nothing on the watchlist is airborne, and switch to watched-flights-only (or a theme change) the moment one appears — the "tell me when THAT plane is out" use case. Rules edited as a list on the web config page, persisted to NVS.
- **Real map underlay for the radar** — investigation notes. Three routes, cheapest first: (a) **simplified vector coastlines/borders**: ship a region-scoped decimated polyline dataset (Natural Earth 10m, simplified to a few thousand points, stored in flash) and `drawLine` it through the same projection as blips — no runtime network, tiny render cost, works at any range; most elegant fit for this hardware. (b) **OSM raster tiles**: fetch 256×256 tiles over HTTP, decode JPEG/PNG on-device (TJpg_Decoder / pngle), cache ~15 tiles in PSRAM for 1024×600, zoom level derived from radar range — real maps but heavy: tile math, decoder, network load on the same core as the ADS-B fetch. (c) **pre-rendered static background** per home location, generated once on a PC and flashed as an image — simplest but not re-locatable. Recommendation: start with (a), treat (b) as a stretch goal once the fetch task has spare budget.
- **Scrolling live-traffic list** — a screen (or sidebar) listing everything currently in the air, sorted by distance: callsign, type, altitude, speed, distance — auto-scrolling, touch to pause/scroll. An extended, live version of Top 5.
- ~~**Animated weather corner**~~ — **DONE** (sun with rotating rays by day, moon + twinkling stars by night, drifting clouds, falling rain/snow, flashing storm; day/night auto-dim with configurable hours + night brightness). Original note: — a nicer corner widget than the static icon: sun with slowly rotating rays, drifting clouds, rain; cross-fades between states as Open-Meteo conditions change. Done as small region updates inside the existing staged redraw, so it costs almost nothing (2Hz furniture, not per-frame work).
- ~~**Phosphor afterglow on the sweep**~~ — **DONE** (per-fb endpoint history, dimmest-first redraw, length + on/off configurable). Original note: — the line leaves a short fading green trail behind it, like a real CRT radar. Pure vibe, big payoff. (S)
- **ATC-style data tags** — realistic controller labels: fixed 3-line format (callsign / FL / speed) with a leader line from the blip, instead of the current stacked labels. (M)
- **Trail heat-map mode** — accumulate 24h of traffic into a density heat map on the radar instead of individual trails. Gorgeous on 1024×600. (M)
- **Ground-traffic auto-filter** — one toggle: hide on-ground aircraft (they're usually airport noise on the display). (S)
- **Closest-approach alert** — flash the border / banner when anything comes within X km of home ("something's RIGHT overhead, go look"). (S)
- **Military/interesting-types highlight** — auto-highlight fighters, tankers, heavies by type code (F-16, KC-135, C-130, A330 MRTT…) — fun for your airspace. (S)
- **"New to me" badge** — keep a persistent registry of every aircraft ever seen; blips get a subtle marker for their first 24h. (M)
- **Buzzer alerts** — tiny piezo on a spare GPIO for watchlist/emergency/closest-approach. Hardware add-on. (M)
- **"Overhead now" screen** — minimal big-text view of only what's within ~5km: callsign, type, altitude, direction. Glanceable when you hear something. (S)
- **Tap-to-pin a target** — touch near a blip to lock Target Intel onto that specific aircraft until you release it. (M)
- **Screensaver clock** — after N minutes idle, full-screen clock with a small live radar in the corner. (S)
- **Location presets** — home/office/cabin slots for lat/lon, switchable from web or touch. (S)
- **Daily digest screen** — today: unique aircraft count, busiest hour, closest pass, highest, fastest + all-time records. (M)
- **History playback** — scrub a timeline to replay the last hour of traffic. (L)
- **Aircraft photos on Target Intel** — fetch by registration from a spotter API and show the actual plane. (L, network-heavy)
- **Self-updating firmware** — "check for update" button on the web page that pulls the latest build from GitHub and flashes itself. (M)
- **Achievement badges** — "1,000 aircraft spotted", "caught a 7700 squawk", "saw 20 at once". Silly, cheap. (M)
- **Cloud-deck dimming** — use METAR ceiling to dim blips you couldn't actually see outside right now. (M, niche aviation-nerd)

### Performance backlog (LCD-7B v2)
